   ...
   //   minChart1 = data[0].x
      ...
      function dragMove(chart, dragDelta){
       ...
       // console.log(scrollPoint)
        ...
        if(dragDelta.offsetX > right ){
          scrollPoint = data.findIndex(item => item.x == new Date(chart.config.options.scales.x.max).setHours(0, 0, 0, 0) ) - 1;
        } 
        if(scrollPoint > data.findIndex(item => item.x == new Date(chart.config.options.scales.x.max).setHours(0, 0, 0, 0)) - 1) {
          scrollPoint = data.findIndex(item => item.x == new Date(chart.config.options.scales.x.max).setHours(0, 0, 0, 0)) - 1
        }
    ...
    if(drag.offsetX > x.getPixelForValue(chart.config.options.scales.x.min) + 11 && 
      ...
      function dragMoveCenter(chart, dragDelta, staticScaleMin, staticScaleMax){
        ...
        const minVal = data.findIndex(item => item.x === staticScaleMax) + difference - range; // 0
        const maxVal = data.findIndex(item => item.x === staticScaleMax) + difference; // 0
       
        let minChart1;
        let maxChart1;
        if (minVal < 0){
          minChart1 = data[0].x;
          maxChart1 = data[range].x ;
             
        } else if (maxVal >= data.length - 1 || difference < 0 && dragDelta.offsetX >= right){
